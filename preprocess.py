...
def clean_number(w):
    return re.sub('[0-9]{1,}([,.]?[0-9]*)*', 'N', w)


def make_vocab(indexer, textfile, seqlength, minseqlength, lowercase, replace_num,
...
def get_batch_sizes(l_location, batchsize):
    curr_idx = 0
    batch_idx = [0]
...
def convert(indexer, textfile, lowercase, replace_num,
    ...
    for tree in open(textfile, 'r'):
        ...
        sent_id += 1
        if sent_id % 100000 == 0:
            print(f"{sent_id}/{num_sents} sentences processed")
    ...
    # get batch sizes
    batch_idx = get_batch_sizes(l_location, batchsize)
    ...
    f = {"source": sents,
         ...
         "batch_l": np.array(batch_l, dtype=int),
         "source_l": np.array(batch_w, dtype=int),
         "sents_l": np.array(sent_l, dtype=int),
         "batch_idx": np.array(batch_idx[:-1], dtype=int),
         "vocab_size": np.array([len(indexer.d)]),
         "idx2word": indexer.idx2word,
         "word2idx": {word: idx for idx, word in indexer.idx2word.items()}, }

    print(f"Saved {len(f['source'])} sentences (dropped {dropped} due to length/unk filter)")
...
def get_data(args):
    ...
    num_sents_train, train_seqlength = make_vocab(indexer, args.trainfile, args.seqlength, args.minseqlength,
                                                  args.lowercase, args.replace_num, 1, 1)
    print(f"Number of sentences in training: {num_sents_train}")
    num_sents_valid, valid_seqlength = make_vocab(indexer, args.valfile, args.seqlength, args.minseqlength,
                                                  args.lowercase, args.replace_num, 0, 0)
    print(f"Number of sentences in valid: {num_sents_valid}")
    num_sents_test, test_seqlength = make_vocab(indexer, args.testfile, args.seqlength, args.minseqlength,
                                                args.lowercase, args.replace_num, 0, 0)
    print(f"Number of sentences in test: {num_sents_test}")
    ...
    print(f"Vocab size: Original = {len(indexer.vocab)}, Pruned = {len(indexer.d)}")
    print(train_seqlength, valid_seqlength, test_seqlength)
    max_sent_l = 0
    max_sent_l = convert(indexer, args.testfile, args.lowercase, args.replace_num,
    ...
    max_sent_l = convert(indexer, args.valfile, args.lowercase, args.replace_num,
                         args.batchsize, valid_seqlength, args.minseqlength,
                         args.outputfile + "-val.pkl", num_sents_valid,
                         max_sent_l, args.shuffle, args.include_boundary, 0)
    max_sent_l = convert(indexer, args.trainfile, args.lowercase, args.replace_num,
    ...
    print(f"Max sent length (before dropping): {max_sent_l}")


def main(arguments):
    parser = argparse.ArgumentParser()
    parser.add_argument('--vocabsize', help="Size of source vocabulary, constructed by taking the top X most frequent words. Rest are replaced with special UNK tokens.", type=int, default=10000)
    parser.add_argument('--vocabminfreq', help="Minimum frequency for vocab. Use this instead of vocabsize if > 0", type=int, default=-1)
    ...
    parser.add_argument('--seqlength', help="Maximum sequence length. Sequences longer than this are dropped.", type=int, default=150)
    parser.add_argument('--minseqlength', help="Minimum sequence length. Sequences shorter than this are dropped.", type=int, default=0)
    parser.add_argument('--outputfile', help="Prefix of the output file names. ", type=str, required=True)
    parser.add_argument('--vocabfile', help="If working with a preset vocab, then including this will ignore srcvocabsize and use the vocab provided here.", type=str, default='')
