...
def get_actions(tree, SHIFT=0, REDUCE=1, OPEN='(', CLOSE=')'):
    ...
    for i in range(len(tree)):
        if tree[i] not in [' ', OPEN, CLOSE]:  # terminal
            if tree[i - 1] in [OPEN, ' ']:
...
def get_random_tree(length, SHIFT=0, REDUCE=1):
    ...
    while len(tree) < 2 * length - 1:
        if (len(stack) >= 2 and num_shift < length and random.random() < 0.5
                or len(stack) < 2):
            tree.append(SHIFT)
            stack.append('')
            num_shift += 1
...
def get_tree_from_binary_matrix(matrix, length):
    ...
    n = len(sent)
    tree = {i: sent[i] for i in range(n)}
...
def get_brackets(tree, idx=0):
    brackets = set()
    if not isinstance(tree, (list, nltk.Tree)):
